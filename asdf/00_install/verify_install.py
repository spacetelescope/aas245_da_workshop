    ...
    if package_name in INSTALLED_PACKAGES:
        ...
        if minimum_version is not None:
            ...
            if installed_version < minimum_version:
                msg = f'"{package_name}" is installed ({installed_version}) but out of date ({minimum_version} required)'
...
if __name__ == "__main__":
    ...
    for package_name, specification in DEPENDENCIES.items():
        if specification is not None:
            ...
            min_version = None
        success, msg = check_package(package_name, minimum_version=min_version)
        if not success:
            errors.append(msg)
    if any(errors):
        print(
